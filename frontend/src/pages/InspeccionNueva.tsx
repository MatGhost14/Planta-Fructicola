...
const InspeccionNueva: React.FC = () => {
  ...
  const handleCapturarFoto = (imageDataUrl: string | string[]) => {
    if (Array.isArray(imageDataUrl)) {
      // Múltiples fotos (ráfaga)
      setFotos([...fotos, ...imageDataUrl]);
      showSuccess(`${imageDataUrl.length} fotos capturadas correctamente`);
    } else {
      // Foto individual
      setFotos([...fotos, imageDataUrl]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const inspeccionData = {
        ...
        temperatura_c: formData.temperatura_c
          ? parseFloat(formData.temperatura_c)
          : undefined,
      ...
      await inspeccionesApi.subirFotos(
        inspeccionCreada.id_inspeccion,
        fotosFiles
      );
      ...
      await inspeccionesApi.subirFirma(
        inspeccionCreada.id_inspeccion,
        firmaFile
      );
